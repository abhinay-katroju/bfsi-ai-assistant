    ...
    def check_safety(query: str) -> Tuple[bool, Optional[str]]:
        ...
        for keyword in SafetyGuardrails.UNSAFE_KEYWORDS:
            if keyword in query_lower:
                return False, f"⚠️ Cannot assist with this query (unsafe/out-of-domain)"
        
        # Check query length
        if len(query) < 3:
            return False, "⚠️ Query too short. Please provide more details."
        
        if len(query) > 1000:
            return False, "⚠️ Query too long. Please keep it concise."
